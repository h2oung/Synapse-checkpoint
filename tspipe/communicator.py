...
class Channel:
    ...
    def send(self, comm: 'Communicator', obj):
        ...
        # print(f"comm.nccl_per_node_ctx: {comm.nccl_per_node_ctx}")
        # print(f"dst_rank: {self.dst_rank}, exists_in_ctx: {self.dst_rank in comm.nccl_per_node_ctx}")
...
class Communicator:
    ...
    #     # # dirty
    #     # sleep(5)
    #     if wait and self.device is not None:
    #         try:
    #             print("Finish wait")
    #             dist.barrier()
    #         except RuntimeError:
    ...
    #     # destroy process group 가능하면 게임에서 해제
    ...
    #         print("Shutting down RPC agent...")
    #         rpc.shutdown()

    #     print("Finish OK")
    #     sleep(5)  # 전체 sync 위해 가능성 있는 idle 시간
    def finish(self, wait=True):
        self.running = False
        print(f"[Rank {self.rank}] Finish call")
        ...
        for thread in [*self.lst_threads_recv_nccl, *self.lst_threads_send_nccl]:
            thread.join()
        print(f"[Rank {self.rank}] NCCL threads joined.")

        # NCCL barrier (GPU 노드만 해당)
        if wait and self.device is not None:
            try:
                print(f"[Rank {self.rank}] Entering NCCL barrier")
                dist.barrier()
                print(f"[Rank {self.rank}] Leaving NCCL barrier")
            except RuntimeError as e:
                print(f"[Rank {self.rank}] Barrier failed: {e}")
