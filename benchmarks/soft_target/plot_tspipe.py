...
for bsz in BATCH_SIZES:
    df = load_csv(
        BASE_DIR / f"baseline-profile-b{bsz}" / f"baseline-profile-b{bsz}" / CSV_NAME
    )
...
plot(
    teacher_results,
    "Per-stage execution time across batch sizes (ViT-Large, TSPipe)",
    "figure10_tspipe_vit_large_teacher.png",
)

plot(
    student_results,
    "Per-stage execution time across batch sizes (ResNet-152, TSPipe)",
    "figure11_tspipe_resnet152_student.png",
)
